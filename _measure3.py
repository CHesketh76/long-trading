...
def simhash(text, bits=64):
    if not text or not text.strip():
        return 0
    counts = Counter(t for t in text.lower() if t.isdigit())
    register = [0] * bits
    for token, weight in counts.items():
        digest = hashlib.sha256(token.encode()).digest()
        h = int.from_bytes(digest[:8], "big")
        for i in range(bits):
            register[i] += (weight if (h >> i) & 1 else -weight) * 0.5
    # alpha tokens at full weight
    counts2 = Counter(t for t in text.lower() if not t.isdigit())
    for token, weight in counts2.items():
        digest = hashlib.sha256(token.encode()).digest()
        h = int.from_bytes(digest[:8], "big")
        for i in range(bits):
