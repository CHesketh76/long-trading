...
EVENTS_COLUMNS = [
    ...
    # --- provenance ---
    ("published_at", "TIMESTAMPTZ"),               # publisher-stated release time (UTC); NULL = unprovable → quarantine
...
def _idx(name: str, on: str, unique: bool = False) -> str:
    """Build a CREATE INDEX statement with IF NOT EXISTS (SQLite-safe)."""
    kind = "UNIQUE " if unique else ""
    return f"CREATE {kind}INDEX IF NOT EXISTS {name} ON {on}"


SCHEMA_INDEX_STATEMENTS = [
    _idx("idx_events_event_ts", "events (event_ts)"),
    _idx("idx_events_source_seq", "events (source_id, sequence)"),
    _idx("idx_events_published_at", "events (published_at) WHERE published_at IS NOT NULL"),
    _idx("idx_events_idempotent", "events (source_id, published_at, url) WHERE url IS NOT NULL", unique=True),
    _idx("idx_event_revisions_event", "event_revisions (event_id)"),
    _idx("idx_event_revisions_rev_at", "event_revisions (revision_at)"),
]
